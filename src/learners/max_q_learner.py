...
class MAXQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.hysteretic_qmix: # OW-QMIX
            ws = th.where(td_error < 0, th.ones_like(td_error)*self.args.weight, ws) # Target is greater than current max
            ...
            ws = th.where(is_max_action | qtot_larger, th.ones_like(td_error)*self.args.weight, ws) # Target is greater than current max
