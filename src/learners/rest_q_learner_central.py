...
class RestQLearnerCentral:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        td_error = (chosen_action_qvals + w_r.detach() * Q_r - (targets.detach()))
        ...
        if self.args.env == "one_step_matrix_game":
            ...
            true_results = th.zeros((matrix_size, matrix_size))       
            res = th.zeros((matrix_size, matrix_size))       
            qr = th.zeros((matrix_size, matrix_size))       
                
            with th.no_grad():
                Qjt = chosen_action_qvals + w_r.detach() * Q_r
                Qtot = chosen_action_qvals
                Qres = rest_chosen_action_qvals
                for i in range(results.shape[0]):
                    for j in range(results.shape[1]):
                        specific_joint_action = th.LongTensor([[[[i], [j]]]]).to(device=mac_out.device).repeat(batch_size, 1, 1, 1) # btn1
                        ...
                        qvals = Qjt[indexes]
                        global_q = qvals.mean()
                        true_results[i][j] = global_q.item()
                        qvals = Qres[indexes]
                        global_q = qvals.mean()
                        res[i][j] = global_q.item()
                        qvals = Q_r[indexes]
                        global_q = qvals.mean()
                        qr[i][j] = global_q.item()
    ...
    def cuda(self):
        ...
        if self.mixer is not None:
            self.mixer.cuda(self.args.cuda_device)
            self.target_mixer.cuda(self.args.cuda_device)
            self.rest_mixer.cuda(self.args.cuda_device)
            self.rest_target_mixer.cuda(self.args.cuda_device)
        if self.central_mac is not None:
            self.central_mac.cuda()
            self.target_central_mac.cuda()
        self.central_mixer.cuda(self.args.cuda_device)
        self.target_central_mixer.cuda(self.args.cuda_device)
