...
from envs.highway_env.envs.common.finite_mdp import compute_ttc_grid
from envs.highway_env.envs.common.graphics import EnvViewer
from envs.highway_env.road.lane import AbstractLane
from envs.highway_env.utils import distance_to_circle, Vector
from envs.highway_env.vehicle.controller import MDPVehicle
from envs.highway_env.vehicle.kinematics import Vehicle
import time

if TYPE_CHECKING:
    from envs.highway_env.envs.common.abstract import AbstractEnv
...
class KinematicWithRouteObservation(ObservationType):
    ...
    def observe(self) -> np.ndarray:
        ...
        # Add ego-vehicle
        # Add nearby traffic
        close_vehicles = self.env.road.close_vehicles_to_multiagent(self.observer_vehicle)
        obs = np.vstack([v.states() for v in close_vehicles])
        if not self.absolute:
            obs[1:] = obs[1:] - obs[0]
        obs = obs.flatten()

        # add route
        timestamps = [0.2, 0.4, 0.6, 0.8, 1]
        predicted_traj = self.observer_vehicle.predict_trajectory_constant_speed(timestamps)
        ori_x = self.observer_vehicle.position[0]
        ori_y = self.observer_vehicle.position[1]
        # ori_h = norm2pi(self.observer_vehicle.heading)
        observe_route = []
        for i in range(len(timestamps)):
            observe_route.append(predicted_traj[0][i][0] - ori_x)
            observe_route.append(predicted_traj[0][i][1] - ori_y)
            # observe_route.append(norm2pi(predicted_traj[1][i]) - ori_h)
        obs = np.concatenate((obs, np.array(observe_route)))
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    ...
    elif config["type"] == "KinematicsWithRoute":
        return KinematicWithRouteObservation(env, **config)
