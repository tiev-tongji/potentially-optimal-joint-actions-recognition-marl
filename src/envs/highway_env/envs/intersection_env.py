...
from envs.highway_env import utils
from envs.highway_env.envs.common.abstract import AbstractEnv, MultiAgentWrapper
from envs.highway_env.road.lane import LineType, StraightLane, CircularLane, AbstractLane
from envs.highway_env.road.regulation import RegulatedRoad
from envs.highway_env.road.road import RoadNetwork
from envs.highway_env.vehicle.kinematics import Vehicle
from envs.highway_env.vehicle.controller import ControlledVehicle
...
class IntersectionEnv(AbstractEnv):
    ...
    def _reward(self, action: int) -> float:
        ...
        # reward = sum(vehicle.speed for vehicle in self.controlled_vehicles) / 40
    ...
    def _info(self, obs: np.ndarray, action: int) -> dict:
        ...
        if all(self.has_arrived(vehicle) for vehicle in self.controlled_vehicles):
            info["all_arrive"] = True
        else:
            info["all_arrive"] = False
        if self.config.get("use_route_traj_points", False):
            route_traj_points = [vehicle.route_traj_points for vehicle in self.controlled_vehicles]
            min_len = np.inf
            for traj in route_traj_points:
                min_len = min(min_len, len(traj))
            for i in range(len(route_traj_points)):
                route_traj_points[i] = route_traj_points[i][:min_len, :]
            route_traj_points = np.array(route_traj_points)
            info["route_traj_points"] = route_traj_points
        return info
    ...
    def _make_road(self) -> None:
        ...
        coordination: x points east, y points south, origin is the center of the intersection
        ...
        for corner in range(4):
            ...
            # ir means entering intersection
            # il means exiting intersection
            # o means some positions far from the intersection
            # Incoming
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            destination_id = self.np_random.randint(4)
            if destination_id == ego_id % 4:
                destination_id = (destination_id + 1) % 4
            destination = "o" + str(destination_id)
            ego_vehicle = self.action_type.vehicle_class(
                             self.road,
                            #  ego_lane.position(80, 0),
                             ego_lane.position(60 + 5*self.np_random.normal(1) + 30 * (ego_id // 4), 0),
            ...
            try:
                ...
                ego_vehicle.set_route(ego_vehicle.route)
                if self.config.get("use_route_traj_points", False):
                    ego_vehicle.get_route_traj_points()
...
register(
    id='intersection-v0',
    entry_point='envs.highway_env.envs:IntersectionEnv',
)

register(
    id='intersection-v1',
    entry_point='envs.highway_env.envs:ContinuousIntersectionEnv',
)

register(
    id='intersection-multi-agent-v0',
    entry_point='envs.highway_env.envs:MultiAgentIntersectionEnv',
)

register(
    id='intersection-multi-agent-v1',
    entry_point='envs.highway_env.envs:TupleMultiAgentIntersectionEnv',
)
