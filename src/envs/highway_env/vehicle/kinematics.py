...
from envs.highway_env.road.road import Road, LaneIndex
from envs.highway_env.vehicle.objects import RoadObject, Obstacle, Landmark
from envs.highway_env.utils import Vector
...
class Vehicle(RoadObject):
    ...
    def on_state_update_simple(self) -> None:
        if self.road:
            if self.lane.after_end(self.position):
                for i, laneid in enumerate(self.route):
                    if self.lane_index[0] == laneid[0] and self.lane_index[1] == laneid[1] and i + 1 < len(self.route):
                        self.lane_index = list(self.route[i + 1])
                        self.lane_index[-1] = 0
                        self.lane_index = tuple(self.lane_index)
                        self.lane = self.road.network.get_lane(self.lane_index)
            if self.road.record_history:
                self.history.appendleft(self.create_from(self))
    ...
    def states(self, origin_vehicle: "Vehicle" = None) -> dict:
        return np.array([self.position[0], self.position[1], self.velocity[0], self.velocity[1], self.heading])
