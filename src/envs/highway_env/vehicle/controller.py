...
from envs.highway_env.road.road import Road, LaneIndex, Route
...
class ControlledVehicle(Vehicle):
    ...
    def get_route_traj_points(self):
        """
        get trajectory points based on self.route
        """
        self.route_traj_points = []
        for lane_index in self.route:
            lane = self.road.network.get_lane(lane_index)
            for delta_s in np.arange(0, lane.length, 0.1):
                self.route_traj_points.append(lane.position(delta_s, 0))
        self.route_traj_points = np.array(self.route_traj_points)
